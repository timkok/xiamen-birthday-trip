...
let currentActiveDay = 1; // 当前激活的天数 (TAB 切换)
...
const menuData = {
  ...
  waldorf: [
    ...
    { name: "冻花生奶 (4杯)", price: 152, desc: "华府甜品师特制冰爽花生乳，解暑佳品。", checked: true, rec: true, tags: ["孩子友好"] }
  ],
  lohkah: [
...
function renderItinerary() {
  ...
  // 默认激活当前选中的 Day (Tab 联动)
  const firstDay = days[0].dayNum;
  if (!days.find(d => d.dayNum === currentActiveDay)) {
    currentActiveDay = firstDay;
  }
  
  setTimeout(() => {
    // 隐藏/显示卡片
    document.querySelectorAll(".day-card").forEach(card => {
      if (card.id === `day-card-${currentActiveDay}`) {
    ...
    document.querySelectorAll(".day-nav-btn").forEach(btn => {
      if (btn.id === `nav-btn-day-${currentActiveDay}`) {
...
function scrollToDay(dayNum) {
  ...
  // 滚动到日程锚点，便于在移动端看清卡片顶部
  const anchor = document.getElementById("itinerary-anchor");
  if (anchor) {
    const offset = 75;
